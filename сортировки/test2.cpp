...
int main() {
    for (unsigned long N = 10; N<=MAX_N; N += 100) {
        Utils util;
        std::cout << N << " ";

        long slow_data[MAX_N], rapid_data[MAX_N];

        //unsigned long const N = 10; //Варьируемый параметр. Можно смело изменять для получения зависимости времени от N. N <= MAX_N
        ...
        //*****************************Тестирование медленной сортировки******************************************************
        {
            ...
            if (!util.check_order(slow_data, N)) {
                ...
                std::cout << t.elapsed() << " ";
                //std::cout << "Performed slow sort of " << N << " elements. Elapsed time: " << t.elapsed()
                          //<< " microseconds.\n";
            }
        ...
        //*****************************Тестирование быстрой сортировки********************************************************
        {
            ...
            if (!util.check_order(rapid_data, N)) {
                ...
                std::cout << t.elapsed() << " ";
                //std::cout << "Performed rapid sort of " << N << " elements. Elapsed time: " << t.elapsed()
                          //<< " microseconds.\n";
            }
